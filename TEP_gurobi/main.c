...
double p1_compute_output ( double *input_i, int stop_decision_i, int coef_length);
...
/******************************************************************************/
{
    ...
    if ( id == 0 )
    {
        ...
        double input_obj_coef[(p-1)*coef_length];// set the objective function coef. array, which contains (p-1)*"coef_legth" element
        double output_outcome[(p-1)];
        int stop_decision;
        int stop_counter;
        ...
        for (stop_counter=0;  ; stop_counter++) // this loop will let program p0_stop_decision to decide ending point.
        {
            ...
            p0_stop_decision( &stop_decision,  stop_counter, end_point);
            printf("stop decision is %d ", stop_decision);
            printf("\n");
            p0_send_decision( p, stop_decision);
            
            if (stop_decision == 0){ //stop_decision = 0 will end program; stop_decision = 1 means program will go on.
                break;}
            
            p0_set_input ( input_obj_coef, p, coef_length ); // p stands for numbers of processes
            
            p0_send_input ( input_obj_coef, p, coef_length );
 
            p0_receive_output ( output_outcome, p );
    ...
    else if ( id > 0 )
    {
        ...
        while (1)
        {
            ...
            if (stop_decision_i == 1)
            {
                ...
                output_i = p1_compute_output ( input_i, stop_decision_i, coef_length);
                
            
                p1_send_output ( output_i, id ); // The desitination is process 0
